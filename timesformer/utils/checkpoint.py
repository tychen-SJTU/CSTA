...
def get_path_to_checkpoint(path_to_job, epoch, age):
    """
    ...
    """
    name = "checkpoint_epoch_{:05d}_age{:02d}.pyth".format(epoch, age)
    return os.path.join(get_checkpoint_dir(path_to_job), name)


def get_path_to_checkpoint_final(path_to_job, age):
    """
    Get the full path to a checkpoint file.
    Args:
        path_to_job (string): the path to the folder of the current job.
        epoch (int): the number of epoch for the checkpoint.
    """
    name = "checkpoint_age{:02d}.pyth".format(age)
    return os.path.join(get_checkpoint_dir(path_to_job), name)
...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, age):
    """
    Save a checkpoint.
    Args:
    ...
    path_to_checkpoint = get_path_to_checkpoint(path_to_job, epoch + 1, age)
    with PathManager.open(path_to_checkpoint, "wb") as f:
        torch.save(checkpoint, f)
    return path_to_checkpoint


def save_final_checkpoint(path_to_job, model, optimizer, cfg, age):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
    ...
    # Write the checkpoint.
    path_to_checkpoint = get_path_to_checkpoint_final(path_to_job, age)
    with PathManager.open(path_to_checkpoint, "wb") as f:
        torch.save(checkpoint, f)
    return path_to_checkpoint
