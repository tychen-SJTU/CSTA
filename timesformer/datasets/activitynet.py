...
class Activitynet(data.Dataset):
    ...
    def _load_image(self, directory, idx, mode):
        try:
                if os.path.exists(os.path.join('./video_data/Activitynet/train', directory,
                                               self.image_tmpl.format(idx))):
                    return [Image.open(os.path.join('./video_data/Activitynet/train', directory,
                                                    self.image_tmpl.format(idx))).convert('RGB')]
                elif os.path.exists(os.path.join('./video_data/Activitynet/removed', directory,
                                                  self.image_tmpl.format(idx))):
                    return [Image.open(os.path.join('./video_data/Activitynet/removed', directory,
                                                    self.image_tmpl.format(idx))).convert('RGB')]
                else:
                    return [Image.open(os.path.join('./video_data/Activitynet/val', directory,
                                                self.image_tmpl.format(idx))).convert('RGB')]
        except Exception:
    ...
    def __getitem__(self, index):
        ...
        if not os.path.exists(full_path):
            full_path = os.path.join('./video_data/Activitynet/val', record.path, file_name)
        if not os.path.exists(full_path):
            full_path = os.path.join('./video_data/Activitynet/removed', record.path, file_name)
    ...
    def get(self, record, indices):
        ...
        for seg_ind in indices:
            ...
            for i in range(self.new_length):
                seg_imgs = self._load_image(record.path, p, self.mode)
