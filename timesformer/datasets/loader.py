...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    Args:
        ...
        split (str): the split of the data loader. Options include `train`,
            `val`, and `test`.
    """
    assert split in ["train", "val", "list", "balance"]
    if split in ["train"]:
        dataset_name = cfg.TRAIN.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = False
        drop_last = True
    elif split in ["balance"]:
        dataset_name = cfg.TRAIN.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = False
        drop_last = True
    ...
    if cfg.MULTIGRID.SHORT_CYCLE and split in (["train"] or ["balance"]) and not is_precise_bn:
        # Create a sampler for multiprocess training
